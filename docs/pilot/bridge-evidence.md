...
| Pinned public key matches Core signing secret | NOT PASSED | The live proof must verify a real Core-signed catalogue against Lens's pinned `dex-core-lens-1` public key. This is the first proof that the public key Dave returned corresponds to the private key sealed in the GitHub secret. A mismatch must fail loudly with a signature error and trigger re-pinning, not gate-on. |
...
- Dave signing-key ceremony: public key returned and pinned; private key material must
  never be pasted into chat. The final run must prove the pinned public key matches the
  sealed Core signing secret by verifying a real Core-signed catalogue.

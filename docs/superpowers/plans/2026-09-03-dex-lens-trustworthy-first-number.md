...
| ~~**critical** — `for_catalogue` omits the six new fields, so a tampered saved ledger passes `report check`~~ **Fixed.** `for_catalogue` now accepts and carries all six run-derived fields (`workflow_graph`, `work_audit`, `expectations`, `strengths`, `reciprocal_lessons`, `workflow_insights`); the reload passes them through and fails closed if re-validation ever loses a digest-bound field; `reports check`/`save` hold the supplied ledger to the digest the report records. Each of the six tampers was reproduced passing the shipped `reports check` before the fix and is refused after it (`tests/reports/test_ledger.py`). Known consequence: a report whose recorded digest predates the widened canonical payload is refused, not silently accepted — re-run the diagnosis | `comparison.py`, `reports/ledger.py`, `reports/cli.py` | Was: affects reload, not the first grade |

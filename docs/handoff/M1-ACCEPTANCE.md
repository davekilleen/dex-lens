...
### 3.8 macOS containment — **two defects behind the red legs, both closed**
...
| Item | Status | Reason |
| --- | --- | --- |
| **macOS `sandbox-exec` containment enforcement** | **MET — on the green `macos-14` legs only; not reproducible on Linux** | `MacOSStrategy` and the shipped `claude_code_containment.sb` profile cannot execute on Linux, where 4 tests skip. They run on darwin, and run [31182628679](https://github.com/davekilleen/dex-capability-exchange/actions/runs/31182628679) is green on both `macos-14` legs with the conformance suite and inventory check also passing there. **The enforcement is real, not a vacuous pass:** the pre-fix darwin failure reported an actual proofs tuple from a completed contained collection (`connect-denied`, `write-open-denied`, `exec-denied`), which is only produced after `sandbox-exec` established containment and the child proved all three denials before reading. Two macOS-specific defects were fixed to get here: the framework-Python exec-set gap (cherry-picked) and the Linux-shaped `socket-denied` proof-label assertion (§3.8). Anyone re-verifying must read the darwin legs — a Linux-only run cannot establish this row. The Fable critique's rule still applies as the fallback: an unproven G1 on a host downgrades that host to guided/export-assisted rather than blocking the pilot, and that downgrade path is itself tested (`honest-fallback`, `TestStrategySelection`) |
